...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (searchToggleBtn && searchBar && searchInput) {
    ...
    searchClearBtn?.addEventListener('click', () => {
      ...
      searchInput.focus();
      if (typeof updateLiveSearch === 'function') updateLiveSearch();
    });
    ...
    searchBar.parentElement.appendChild(searchDropdown);
    searchDropdown.addEventListener('click', (e) => e.stopPropagation());

    function getAllSearchableProducts() {
      let all = [];
      if (typeof categoryConfigs !== 'undefined') {
        Object.values(categoryConfigs).forEach(cat => {
          if (cat.products) {
            cat.products.forEach(p => {
              if (!all.some(cp => cp.id === p.id)) all.push(p);
            });
          }
        });
      }
      if (typeof dynamicProducts !== 'undefined' && Array.isArray(dynamicProducts)) {
        dynamicProducts.forEach(p => {
          if (!all.some(cp => cp.id === p.id)) {
            all.push({
              id: p.id,
              name: p.name,
              length: p.length_in,
    ...
    function updateLiveSearch() {
      ...
      const allProds = getAllSearchableProducts();
      const filtered = allProds.filter(p => {
        const sizeStr = `${p.length}x${p.width}x${p.height}`.toLowerCase();
    ...
    searchInput.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        ...
        if (isHomePage) {
          ...
          // Redirect to catalog page with search parameter (clean URL, no .html)
          window.location.href = `catalog?search=${encodeURIComponent(query)}`;
        }
      }
    });

    // Read URL search parameter and pre-populate search inputs
    ...
    if (searchParam) {
      ...
      // Also pre-populate the catalog-specific search inputs if they exist
      const catalogSearchInput = document.getElementById('catalog-search');
      if (catalogSearchInput) {
  ...
  function filterAndRender() {
    ...
    // If a search query is present but no explicit category type in URL, search across ALL categories
    const urlType = new URLSearchParams(window.location.search).get('type') || new URLSearchParams(window.location.search).get('category');
    const currentSearchQuery = searchInput?.value.trim().toLowerCase();
    const searchAllCategories = currentSearchQuery && !urlType;

    let combined = [];
    if (searchAllCategories) {
      // Collect products from ALL categories for global search
      Object.values(categoryConfigs).forEach(cat => {
        cat.products.forEach(p => {
      ...
      combined = [...configProducts];
    }

    // Combine with dynamicProducts matching activeCategory (or all if global search)
    if (dynamicProducts && dynamicProducts.length > 0) {
      const matched = searchAllCategories
        ? dynamicProducts
        : dynamicProducts.filter(p => p.categorySlug === activeCategory || p.categoryId === activeCategory);
      matched.forEach(p => {
    ...
    // 5. Header Search filter — match name, size string, and category keyword
    if (currentSearchQuery) {
      filtered = filtered.filter(p => {
        const sizeStr = `${p.length}x${p.width}x${p.height}`.toLowerCase();
        const catStr = (p.category || '').toLowerCase();
        return p.name.toLowerCase().includes(currentSearchQuery)
          || sizeStr.includes(currentSearchQuery)
          || catStr.includes(currentSearchQuery);
      });
    }
    ...
    grid.innerHTML = filtered.map(p => {
      ...
      return `
        <div class="premium-prod-card" id="card-${p.id}">
          <div class="premium-card-img-wrap" style="cursor:pointer" onclick="window.location.href='product-detail?id=${p.id}'">
            <img src="${p.image}" alt="${p.name}">
          </div>
          <div class="premium-card-body">
            <h3 class="premium-card-title" style="cursor:pointer" onclick="window.location.href='product-detail?id=${p.id}'">${p.name}</h3>
  ...
  function initDynamicProductDetail() {
    ...
    let product = dynamicProducts.find(p => 
    ...
    if (!product) {
      for (const catKey in categoryConfigs) {
        const item = categoryConfigs[catKey].products.find(p => p.id && p.id.toLowerCase() === lowerId);
        if (item) {
    ...
    // No unsafe fallback — if product not found, show a clear error state
    if (!product) {
      container.innerHTML = `
        <div style="grid-column:1/-1; text-align:center; padding:80px 20px;">
          <h3 style="font-family:var(--font-head); color:var(--charcoal); margin-bottom:12px;">Product Not Found</h3>
          <p style="color:var(--charcoal-xlt); margin-bottom:20px;">We couldn't find the product you're looking for.</p>
          <a href="products.html" style="display:inline-block; padding:10px 24px; background:var(--brown); color:#fff; border-radius:8px; text-decoration:none; font-weight:600;">Browse All Products</a>
        </div>
      `;
      return;
    }
